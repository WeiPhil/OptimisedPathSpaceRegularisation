...
# Make sure you have compiled mitsuba in the required 'gpu_autodiff_rgb' mode
...
def create_multi_sensor_xml(filename,original_size : np.array,subcrop_size: np.array, main_crop_size : np.array,main_crop_offset : np.array,filter='gaussian',pixel_format='rgb'):
    ...
    assert np.all(num_sensors_xy == num_sensors_xy.astype(np.int)) , "Cannot tile the main_crop with that subcrop size"
    num_sensors_xy = num_sensors_xy.astype(np.int)

    main_crop_size = main_crop_size.astype(np.int)
    main_crop_offset = main_crop_offset.astype(np.int)
    subcrop_size = subcrop_size.astype(np.int)
    ...
    for i,(subcrop_offset_x,subcrop_offset_y) in enumerate(crop_offsets):
        ...
        film.tail = '\n\t\t\t\t'
        ET.SubElement(film,"integer", name="width",value=str(original_size[0])).tail = '\n\t\t\t\t'
        ET.SubElement(film,"integer", name="height",value=str(original_size[1])).tail = '\n\t\t\t\t'
        ET.SubElement(film,"integer", name="crop_offset_x",value=str(subcrop_offset_x)).tail = '\n\t\t\t\t'
        ET.SubElement(film,"integer", name="crop_offset_y",value=str(subcrop_offset_y)).tail = '\n\t\t\t\t'
        ET.SubElement(film,"integer", name="crop_width",value=str(subcrop_size[0])).tail = '\n\t\t\t\t'
        ET.SubElement(film,"integer", name="crop_height",value=str(subcrop_size[1])).tail = '\n\t\t\t\t'
        ET.SubElement(film,"string", name="pixel_format",value=pixel_format).tail = '\n\t\t\t\t'
...
def cropped_ref_image(bitmap,sensor_idx):
    np_bmp = np.array(bitmap).astype(np.float)
    x,y = sensor_idx % num_sensors_xy[0],sensor_idx // num_sensors_xy[0]
    start_offset = main_crop_offset[0]+subcrop_size[0]*x,main_crop_offset[1]+subcrop_size[1]*y
    image_ref_crop = ek.cuda_autodiff.Float32(np.array(bitmap).astype(np.float)[start_offset[1]:start_offset[1]+subcrop_size[1],start_offset[0]:start_offset[0]+subcrop_size[0],:3].flatten())
...
# both have big impact on gpu memory usage. The following settings will require about !14GB!
# of gpu memory. 
spp = 1024
...
for scene_folder in scene_folders:
    Thread.thread().file_resolver().append(main_scene_folder + scene_folder)
...
for multi_sensor_scene_xml in scenes_multi_sensor_xmls:
    scene = load_string(multi_sensor_scene_xml,spp=spp,size=original_size[0],res=roughness_res,depth=depth,integrator=integrator)
    scenes.append(scene)

# assuming multi_sensor was successfully created
num_sensors_xy = (main_crop_size/subcrop_size).astype(np.int)

num_sensors = ek.hprod(num_sensors_xy)
...
if start_iter == 0:
    ...
    for scene in scenes:
        ...
        params.keep([att_factors_2d_str,att_factors_3d_str,att_factors_4d_str,att_factors_5d_str])
        att_factors_5d = np.ones((roughness_res,roughness_res,roughness_res,roughness_res,roughness_res)).flatten() - 0.6
        att_factors_4d = np.ones((roughness_res,roughness_res,roughness_res,roughness_res)).flatten() - 0.6
        att_factors_3d = np.ones((roughness_res,roughness_res,roughness_res)).flatten() - 0.6
        att_factors_2d = np.ones((roughness_res,roughness_res)).flatten() - 0.6

        # last vertex diffuse, don't add any bias, easy connection
        att_factors_2d.reshape([roughness_res,roughness_res])[-1,:] = 0
        att_factors_3d.reshape([roughness_res,roughness_res,roughness_res])[-1,:,:] = 0
        att_factors_4d.reshape([roughness_res,roughness_res,roughness_res,roughness_res])[-1:,:,:] = 0
        att_factors_5d.reshape([roughness_res,roughness_res,roughness_res,roughness_res,roughness_res])[-1:,:,:,:,:] = 0
    ...
    att_factors_5d = np.load('numpy_data/opsr_path_interp-att_factors_5d-beta_{}_spp_{}.npy'.format(beta,spp))[-1]
    att_factors_4d = np.load('numpy_data/opsr_path_interp-att_factors_4d-beta_{}_spp_{}.npy'.format(beta,spp))[-1]
    att_factors_3d = np.load('numpy_data/opsr_path_interp-att_factors_3d-beta_{}_spp_{}.npy'.format(beta,spp))[-1]
    att_factors_2d = np.load('numpy_data/opsr_path_interp-att_factors_2d-beta_{}_spp_{}.npy'.format(beta,spp))[-1]
...
for scene_ref_path in scene_ref_paths:
    ref_bitmap = Bitmap(main_scene_folder + scene_ref_path + '_{}.exr'.format(original_size[0]))
...
def add_to_gathered(gathered,ek_image,sensor_idx):
    x,y = sensor_idx % num_sensors_xy[0],sensor_idx // num_sensors_xy[0]
    start_offset = main_crop_offset[0]+subcrop_size[0]*x,main_crop_offset[1]+subcrop_size[1]*y
    gathered[start_offset[1]:start_offset[1]+subcrop_size[1],start_offset[0]:start_offset[0]+subcrop_size[0],:3] = np.array(ek_image).reshape((*subcrop_size,3))

# We write some info to a log file at every iteration.
with open('optimization_log.txt','w') as log_file:
...
def run_optimization(start_iter=0,end_iter=iterations,opt=None):
    # Assume we can load the old data to continue from a checkpoint
    if start_iter != 0:
        param_data_5d = list(np.load('numpy_data/opsr_path_interp-att_factors_5d-beta_{}_spp_{}.npy'.format(beta,spp)))
        param_data_4d = list(np.load('numpy_data/opsr_path_interp-att_factors_4d-beta_{}_spp_{}.npy'.format(beta,spp)))
        param_data_3d = list(np.load('numpy_data/opsr_path_interp-att_factors_3d-beta_{}_spp_{}.npy'.format(beta,spp)))
        param_data_2d = list(np.load('numpy_data/opsr_path_interp-att_factors_2d-beta_{}_spp_{}.npy'.format(beta,spp)))
        loss_data_sensors,image_loss_data_sensors,var_data_sensors = np.load('numpy_data/opsr_path_interp-losses-beta_{}_spp_{}.npy'.format(beta,spp))
    else:
        param_data_5d = []
        param_data_4d = []
        param_data_3d = []
        param_data_2d = []
        loss_data_sensors = -np.ones((len(scenes),end_iter - start_iter,num_sensors))
        image_loss_data_sensors = -np.ones((len(scenes),end_iter - start_iter,num_sensors))
        var_data_sensors = -np.ones((len(scenes),end_iter - start_iter,num_sensors))
    ...
    for num_scene in range(num_scenes):
        sensor_indexes.append(list(np.random.permutation(np.arange(num_sensors))))

    scene_indexes = list(np.random.permutation((np.arange(num_scenes))))

    for i in range(0,end_iter - start_iter):
        print("Optimization Progress : {} from {}".format(start_iter + i,end_iter), end="\r")
        ...
        if not sensor_indexes[scene_idx]:
            sensor_indexes[scene_idx] = list(np.random.permutation((np.arange(num_sensors))))

        sensor_idx = sensor_indexes[scene_idx].pop()

        # initialise optimizer for the first time
        if(opt == None):
            opt = Adam(params_scenes[scene_idx],lr=0.0005)
        else:
            ...
            opt.params = params_scenes[scene_idx]

        # opt.params[att_factors_5d_str]
        crop_size = scenes[scene_idx].sensors()[sensor_idx].film().crop_size()

        image = render(scenes[scene_idx], optimizer=opt, unbiased=True, spp=spp,sensor_index=sensor_idx)

        # decomposition of the image in the different chanels
        aov_r = image[0::9]
        aov_g = image[1::9]
        aov_b = image[2::9]

        # second moment rgb channels
        aov_r_m2 = image[6::9]
        aov_g_m2 = image[7::9]
        aov_b_m2 = image[8::9]

        # recompose the rgb image from the corresponding channels.
        aov_rgb = ek.cuda_autodiff.Float32.zero(ek.hprod(crop_size)*3)
        aov_rgb[0::3] = aov_r
        aov_rgb[1::3] = aov_g
        aov_rgb[2::3] = aov_b

        # recompose the 2nd moment rgb image from the corresponding channels
        aov_rgb_m2 = ek.cuda_autodiff.Float32.zero(ek.hprod(crop_size)*3)
        aov_rgb_m2[0::3] = aov_r_m2
        aov_rgb_m2[1::3] = aov_g_m2
        aov_rgb_m2[2::3] = aov_b_m2

        # Add each crop to the gathered image for visualisation
        add_to_gathered(gathered_rgb_images[scene_idx],aov_rgb,sensor_idx)
        add_to_gathered(gathered_rgb_m2_images[scene_idx],aov_rgb_m2,sensor_idx)
        ...
        variance = aov_rgb_m2 - ek.sqr(aov_rgb)
        image_loss = ek.hsum(ek.abs(image_ref - aov_rgb))/ len(image_ref)
        var_loss  = ek.hsum(variance)/len(variance)
        mean_energy_rcp = ek.rcp(ek.hsum(image_ref)/ len(image_ref))
        ...
        write_bitmap(work_folder + output_folder + 'out_{}.exr'.format(i + start_iter), aov_rgb, crop_size)
        write_bitmap(work_folder + output_folder + 'var_out_{}.exr'.format(i + start_iter), variance, crop_size)
        write_bitmap('last_gathered_rgb_{}.exr'.format(scene_idx) ,ek.cuda_autodiff.Float32(gathered_rgb_images[scene_idx].flatten()),main_crop_size)
        write_bitmap(work_folder + gathered_output_folder + 'gathered_rgb_{}_{}.exr'.format(scene_idx,i+start_iter) ,ek.cuda_autodiff.Float32(gathered_rgb_images[scene_idx].flatten()),main_crop_size)
        write_bitmap(work_folder + gathered_output_folder +'gathered_rgb_m2_{}.exr'.format(scene_idx),ek.cuda_autodiff.Float32(gathered_rgb_m2_images[scene_idx].flatten()),main_crop_size)
        ...
        with open('optimization_log.txt','a') as log_file:
            for key,value in params_scenes[scene_idx].items():
                log_file.write('Gradient for '+ key + ' is ' + str(ek.gradient(params_scenes[scene_idx][key]))+'\n')
        ...
        with open('optimization_log.txt','a') as log_file:
            for key,value in params_scenes[scene_idx].items():
                if(ek.any(ek.isnan(ek.gradient(params_scenes[scene_idx][key]))) == True):
                    log_file.write(str(ek.gradient(params_scenes[scene_idx][key])) + '\n')
                    log_file.write('/!\\ Gradient for '+ key + ' is nan, setting it to zero\n' %it)
                    ek.set_gradient(params_scenes[scene_idx][key],[0,0,0])
                if(ek.any(params_scenes[scene_idx][key] < 0.005)):
                    params_scenes[scene_idx][key] = ek.max(params_scenes[scene_idx][key],0.005)
                    log_file.write("/!\\ Clamped the value of " + key + ' to ' + str(params_scenes[scene_idx][key]) + '\n')
                    params_scenes[scene_idx].update()
                if(ek.any(params_scenes[scene_idx][key] > 0.999)):
                    params_scenes[scene_idx][key] = ek.min(params_scenes[scene_idx][key],0.999)
                    log_file.write("/!\\ Value higher than 1 " + key + ' to ' + str(params_scenes[scene_idx][key]) + '\n')
                    params_scenes[scene_idx].update()
                if(ek.any(ek.isnan(params_scenes[scene_idx][key]))):
                    log_file.write("/!\\ Encountered nan! Replaced by initial value \n")
                    params_scenes[scene_idx][key][ek.isnan(params_scenes[scene_idx][key])] = 0.4
                    params_scenes[scene_idx].update()


        param_data_5d.append(np.copy(params_scenes[scene_idx][att_factors_5d_str]))
        param_data_4d.append(np.copy(params_scenes[scene_idx][att_factors_4d_str]))
        param_data_3d.append(np.copy(params_scenes[scene_idx][att_factors_3d_str]))
        param_data_2d.append(np.copy(params_scenes[scene_idx][att_factors_2d_str]))
        # Compare iterate against ground-truth value
        # err_ref = ek.hsum(ek.sqr(param_ref - params_scenes[scene_idx]['water.alpha.value']))
        with open('optimization_log.txt','a') as log_file:
            for key,value in params_scenes[scene_idx].items():
                log_file.write('New params: '+ key + ' : ' + str(value) + '\n')

        # save at each iteration to easilly continue or backup if needed
        np.save('numpy_data/opsr_path_interp-att_factors_5d-beta_{}_spp_{}'.format(beta,spp),param_data_5d)
        np.save('numpy_data/opsr_path_interp-att_factors_4d-beta_{}_spp_{}'.format(beta,spp),param_data_4d)
        np.save('numpy_data/opsr_path_interp-att_factors_3d-beta_{}_spp_{}'.format(beta,spp),param_data_3d)
        np.save('numpy_data/opsr_path_interp-att_factors_2d-beta_{}_spp_{}'.format(beta,spp),param_data_2d)

        np.save('numpy_data/opsr_path_interp-losses-beta_{}_spp_{}'.format(beta,spp),[loss_data_sensors,image_loss_data_sensors,var_data_sensors])
...
optimizer,loss_data_sensors,image_loss_data_sensors,var_data_sensors,param_data_5d,param_data_4d,param_data_3d,param_data_2d = run_optimization(start_iter=start_iter,end_iter=start_iter + iterations)
