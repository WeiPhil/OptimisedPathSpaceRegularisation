 ...
 * - raw
   ...
   - Should the transformation to the stored color data (e.g. sRGB to linear,
     ...
     with bitmaps storing normal maps that use a linear encoding. (Default:
false)
...
protected:
    ...
    template <uint32_t Channels, bool Raw> Object *expand_3() const {
        ...
        return new BitmapTextureImpl<Float, Spectrum, Channels, Raw>(
            props, m_bitmap, m_name, m_transform, m_mean, m_filter_type,
            m_wrap_mode);
    }
...
public:
    ...
    BitmapTextureImpl(const Properties &props, const Bitmap *bitmap,
                      const std::string &name,
                      const ScalarTransform3f &transform, ScalarFloat mean,
                      FilterType filter_type, WrapMode wrap_mode)
        : Texture(props), m_resolution(ScalarVector2i(bitmap->size())),
          m_inv_resolution_x((int) bitmap->width()),
          m_inv_resolution_y((int) bitmap->height()), m_name(name),
          m_transform(transform), m_mean(mean), m_filter_type(filter_type),
    ...
    Vector2f eval_1_grad(const SurfaceInteraction3f &si,
        ...
        if constexpr (Channels == 3 && is_spectral_v<Spectrum> && !Raw) {
            ...
            if (m_filter_type == FilterType::Bilinear) {
                ...
                using StorageType =
                    std::conditional_t<Channels == 1, Float, Color3f>;
                ...
                Float f00 = convert_to_monochrome(
                    gather<StorageType>(m_data, index.x(), active));
                Float f10 = convert_to_monochrome(
                    gather<StorageType>(m_data, index.y(), active));
                Float f01 = convert_to_monochrome(
                    gather<StorageType>(m_data, index.z(), active));
                Float f11 = convert_to_monochrome(
                    gather<StorageType>(m_data, index.w(), active));
                ...
                Vector2f df_uv{ uv_tm.coeff(0, 0) * df_xy.x() +
                                    uv_tm.coeff(1, 0) * df_xy.y(),
                                uv_tm.coeff(0, 1) * df_xy.x() +
                                    uv_tm.coeff(1, 1) * df_xy.y() };
    ...
    MTS_INLINE auto interpolate(const SurfaceInteraction3f &si,
                                Mask active) const {
        // Storage representation underlying this texture
        ...
        if (m_filter_type == FilterType::Bilinear) {
            ...
            // Integer pixel positions for bilinear interpolation
            Vector2i uv_i = floor2int<Vector2i>(uv), uv_i_w = wrap(uv_i);
    ...
    parameters_changed(const std::vector<std::string> &keys = {}) override {
        if (keys.empty() || string::contains(keys, "data")) {
            /// Convert m_data into a managed array (available in CPU/GPU
            /// address space)
...
template <typename Float, typename Spectrum, uint32_t Channels, bool Raw>
Class *BitmapTextureImpl<Float, Spectrum, Channels, Raw>::m_class = new Class(
    detail::bitmap_class_name<Channels, Raw>(), "Texture",
