...
material name. (Default: bk7 / 1.5046)
 * - ext_ior
   - |float| or |string|
 ...
 * - specular_reflectance, specular_transmittance
   - |spectrum| or |texture|
   - Optional factor that can be used to modulate the specular
reflection/transmission components. Note that for physical realism, these
parameters should never be touched. (Default: 1.0)
...
parameter is equal to the *root mean square* (RMS) slope of the microfacets.
:monosp:`alpha` is a convenience parameter to initialize both :monosp:`alpha_u`
and :monosp:`alpha_v` to the same value. (Default: 0.1)
 * - sample_visible
   - |bool|
...
important off-specular reflections peaks observed in real-world measurements of
such materials.

.. subfigstart::
.. subfigure::
../../resources/data/docs/images/render/bsdf_roughdielectric_glass.jpg :caption:
Anti-glare glass (Beckmann, :math:`\alpha=0.02`)
.. subfigure::
../../resources/data/docs/images/render/bsdf_roughdielectric_rough.jpg :caption:
Rough glass (Beckmann, :math:`\alpha=0.1`)
.. subfigure::
../../resources/data/docs/images/render/bsdf_roughdielectric_textured.jpg
    :caption: Rough glass with textured alpha
.. subfigend::
    :label: fig-bsdf-roughdielectric

This plugin is essentially the *roughened* equivalent of the (smooth) plugin
:ref:`dielectric <bsdf-dielectric>`. For very low values of :math:`\alpha`, the
...
where *exterior* refers to the side that contains the surface normal. Similar to
the :ref:`dielectric <bsdf-dielectric>` plugin, IOR values can either be
specified numerically, or based on a list of known materials (see the
...
public:
    ...
    RoughDielectric(const Properties &props) : Base(props) {
        ...
        BSDFFlags extra =
            (m_alpha_u != m_alpha_v) ? BSDFFlags::Anisotropic : BSDFFlags(0);
    ...
    sample(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        /* Construct the microfacet distribution matching the roughness values
         * at the current surface position. */
        ...
        bs.sampled_type =
            select(selected_r, UInt32(+BSDFFlags::GlossyReflection),
        ...
        if (any_or<true>(selected_t)) {
            ...
            UnpolarizedSpectrum factor = (ctx.mode == TransportMode::Radiance)
                                             ? sqr(eta_ti)
                                             : Float(1.f);
    ...
    // #Add by Weier
    std::pair<BSDFSample3f, Spectrum>
    sample_rough(const BSDFContext &ctx, const SurfaceInteraction3f &si,
                 Float sample1, const Point2f &sample2,
                 const Vector2f &acc_roughness, Mask active) const override {
        MTS_MASKED_FUNCTION(ProfilerPhase::BSDFSample, active);
        ...
        /* Construct the microfacet distribution matching the roughness values
         * at the current surface position. */
        MicrofacetDistribution distr(m_type, acc_roughness.x(),
                                     acc_roughness.y(), m_sample_visible);
    ...
    Spectrum eval(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        Float eta = select(cos_theta_i > 0.f, Float(m_eta), Float(m_inv_eta)),
              inv_eta =
                  select(cos_theta_i > 0.f, Float(m_inv_eta), Float(m_eta));

        // Compute the half-vector
        Vector3f m = normalize(si.wi + wo * select(reflect, Float(1.f), eta));

        // Ensure that the half-vector points into the same hemisphere as the
        // macrosurface normal
        ...
        if (any_or<true>(eval_t)) {
            ...
            Float scale = (ctx.mode == TransportMode::Radiance) ? sqr(inv_eta)
                                                                : Float(1.f);

            // Compute the total amount of transmission
            UnpolarizedSpectrum value =
                abs((scale * (1.f - F) * D * G * eta * eta * dot(si.wi, m) *
                     dot(wo, m)) /
    ...
    // #Add by Weier
    Spectrum eval_rough(const BSDFContext &ctx, const SurfaceInteraction3f &si,
                        const Vector3f &wo, const Vector2f &acc_roughness,
    ...
    Float pdf(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        MicrofacetDistribution sample_distr(
            m_type, m_alpha_u->eval_1(si, active),
            m_alpha_v->eval_1(si, active), m_sample_visible);
        ...
        if (unlikely(!m_sample_visible))
            sample_distr.scale_alpha(
                1.2f - .2f * sqrt(abs(Frame3f::cos_theta(si.wi))));

        // Evaluate the microfacet model sampling density function
        Float prob =
            sample_distr.pdf(mulsign(si.wi, Frame3f::cos_theta(si.wi)), m);
    ...
    // #Add by Weier
    Float pdf_rough(const BSDFContext &ctx, const SurfaceInteraction3f &si,
                    const Vector3f &wo, const Vector2f &acc_roughness,
                    Mask active) const override {
        MTS_MASKED_FUNCTION(ProfilerPhase::BSDFEvaluate, active);
        ...
        /* Construct the microfacet distribution matching the
           roughness values at the current surface position. */
        MicrofacetDistribution sample_distr(
            m_type, acc_roughness.x(), acc_roughness.y(), m_sample_visible);
    ...
    // #Add by Weier
    Vector2f get_roughness(const SurfaceInteraction3f &si,
                           Mask active) const override {
        return Vector2f(m_alpha_u->eval_1(si, active),
                        m_alpha_v->eval_1(si, active));
    }

    // #Add by Weier
    Float get_eta(const SurfaceInteraction3f & /*si*/,
                  Mask /*active*/) const override {
        return Float(m_eta);
    }

    // #Add by Weier
    Float get_inv_eta(const SurfaceInteraction3f & /*si*/,
                      Mask /*active*/) const override {
        return Float(m_inv_eta);
    }
    ...
    std::string to_string() const override {
        ...
        oss << "RoughDielectric[" << std::endl
            ...
            << "  sample_visible = " << (int) m_sample_visible << ","
            << std::endl;

        if (!has_flag(m_flags, BSDFFlags::Anisotropic)) {
            oss << "  alpha = "                  << string::indent(m_alpha_v) << "," << std::endl;
        } else {
            oss << "  alpha_u = " << string::indent(m_alpha_u) << ","
                << std::endl
                << "  alpha_v = " << string::indent(m_alpha_v) << ","
                << std::endl;
        }
