...
public:
    ...
    // #Add by Weier
    std::pair<BSDFSample3f, Spectrum> sample_rough(const BSDFContext &ctx,
                                             const SurfaceInteraction3f &si,
                                             Float sample1,
                                             const Point2f &sample2,
        ...
        if (any_or<true>(sample_specular)) {
            MicrofacetDistribution distr(
                m_type, 0.5f * (acc_roughness.x() + acc_roughness.y()),
                m_sample_visible);
            Normal3f m = std::get<0>(distr.sample(si.wi, sample2));
    ...
    Spectrum eval_rough(const BSDFContext &ctx, const SurfaceInteraction3f &si,
                  const Vector3f &wo, const Vector2f &acc_roughness, Mask active) const override {
        MTS_MASKED_FUNCTION(ProfilerPhase::BSDFEvaluate, active);
    ...
    Vector2f get_roughness(const SurfaceInteraction3f &/*si*/,
                           Mask /*active*/) const override {
        return Vector2f(m_alpha,m_alpha);
    }
