...
Bump mapping is a simple technique for cheaply adding surface detail to a
rendering. This is done by perturbing the shading coordinate frame based on a
displacement height field provided as a texture. This method can lend objects a
highly realistic and detailed appearance (e.g. wrinkled or covered by scratches
and other imperfections) without requiring any changes to the input geometry.
The implementation in Mitsuba uses the common approach of ignoring the usually
negligible texture-space derivative of the base mesh surface normal. As side
effect of this decision, it is invariant to constant offsets in the height field
texture: only variations in its luminance cause changes to the shading frame.
Note that the magnitude of the height field variations influences the scale of
the displacement.
.. subfigstart::
...
public:
    ...
    BumpMap(const Properties &props) : Base(props) {
        for (auto &[name, obj] : props.objects(false)) {
            ...
            if (texture) {
                if (m_nested_texture)
                    Throw(
                        "Only a single Texture child object can be specified.");
    ...
    sample(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        auto [bs, weight] =
            m_nested_bsdf->sample(ctx, perturbed_si, sample1, sample2, active);
        ...
        active &=
            Frame3f::cos_theta(bs.wo) * Frame3f::cos_theta(perturbed_wo) > 0.f;
        bs.wo = perturbed_wo;
    ...
    sample_rough(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        perturbed_si.sh_frame = frame(si, active);
        perturbed_si.wi       = perturbed_si.to_local(si.wi);
        auto [bs, weight]     = m_nested_bsdf->sample_rough(
            ctx, perturbed_si, sample1, sample2, acc_roughness, active);
        active &= any(neq(depolarize(weight), 0.f));
    ...
    Spectrum eval_rough(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        return select(active,
                      m_nested_bsdf->eval_rough(ctx, perturbed_si, perturbed_wo,
    ...
    Float pdf_rough(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        return select(active,
                      m_nested_bsdf->pdf_rough(ctx, perturbed_si, perturbed_wo,
                                               acc_roughness, active),
                      0.f);
    }
    ...
    Frame3f frame(const SurfaceInteraction3f &si, Mask active) const {
        ...
        Vector3f dp_du =
            fmadd(si.sh_frame.n, grad_uv.x() - dot(si.sh_frame.n, si.dp_du),
                  si.dp_du);
        Vector3f dp_dv =
            fmadd(si.sh_frame.n, grad_uv.y() - dot(si.sh_frame.n, si.dp_dv),
                  si.dp_dv);
    ...
    Vector2f get_roughness(const SurfaceInteraction3f &si,
                           Mask active) const override {
        return m_nested_bsdf->get_roughness(si, active);
    }
    ...
    std::string to_string() const override {
        ...
        oss << "BumpMap[" << std::endl
            ...
            << "  nested_texture = " << string::indent(m_nested_texture) << ","
            << std::endl
