 ...
 * - (Nested plugin)
   - |bsdf|
   - A nested BRDF that should be turned into a two-sided scattering model. If
two BRDFs are specified, they will be placed on the front and back side,
respectively

.. subfigstart::
.. subfigure::
../../resources/data/docs/images/render/bsdf_twosided_cbox_onesided.jpg
   :caption: From this angle, the Cornell box scene shows visible back-facing
geometry
...
public:
    ...
    sample(const BSDFContext &ctx_, const SurfaceInteraction3f &si_,
    ...
    // #Add by Weier
    std::pair<BSDFSample3f, Spectrum>
    sample_rough(const BSDFContext &ctx_, const SurfaceInteraction3f &si_,
                 Float sample1, const Point2f &sample2,
                 const Vector2f &acc_roughness,
        ...
        Result result = zero<Result>();
        if (any_or<true>(front_side))
            masked(result, front_side) = m_brdf[0]->sample_rough(
                ctx, si, sample1, sample2, acc_roughness, front_side);
        ...
        if (any_or<true>(back_side)) {
            ...
            si.wi.z() *= -1.f;
            masked(result, back_side) = m_brdf[1]->sample_rough(
                ctx, si, sample1, sample2, acc_roughness, back_side);
            masked(result.first.wo.z(), back_side) *= -1.f;
        }
    ...
    // #Add by Weier
    Spectrum eval_rough(const BSDFContext &ctx_,
                        const SurfaceInteraction3f &si_, const Vector3f &wo_,
                        const Vector2f &acc_roughness,
                        Mask active = true) const override {
        MTS_MASKED_FUNCTION(ProfilerPhase::BSDFEvaluate, active);
        ...
        if (any_or<true>(front_side))
            result =
                m_brdf[0]->eval_rough(ctx, si, wo, acc_roughness, front_side);
        ...
        if (any_or<true>(back_side)) {
            ...
            masked(result, back_side) =
                m_brdf[1]->eval_rough(ctx, si, wo, acc_roughness, back_side);
        }
    ...
    // #Add by Weier
    Float pdf_rough(const BSDFContext &ctx_, const SurfaceInteraction3f &si_,
                    const Vector3f &wo_, const Vector2f &acc_roughness,
                    Mask active = true) const override {
        MTS_MASKED_FUNCTION(ProfilerPhase::BSDFEvaluate, active);
        ...
        if (any_or<true>(front_side))
            result =
                m_brdf[0]->pdf_rough(ctx, si, wo, acc_roughness, front_side);
        ...
        if (any_or<true>(back_side)) {
            ...
            masked(result, back_side) =
                m_brdf[1]->pdf_rough(ctx, si, wo, acc_roughness, back_side);
        }
    ...
    // #Add by Weier
    Vector2f get_roughness(const SurfaceInteraction3f &si_,
                           Mask active) const override {

        MTS_MASKED_FUNCTION(ProfilerPhase::BSDFEvaluate, active);

        SurfaceInteraction3f si(si_);
        Vector2f roughness(0.f);

        Mask front_side = Frame3f::cos_theta(si.wi) > 0.f && active,
             back_side  = Frame3f::cos_theta(si.wi) < 0.f && active;

        if (any_or<true>(front_side))
            roughness = m_brdf[0]->get_roughness(si, front_side);

        if (any_or<true>(back_side)) {
            /* Probably useless since what interests us here is only
               the si.p for evaluation any possible texture */
            si.wi.z() *= -1.f;

            masked(roughness, back_side) =
                m_brdf[1]->get_roughness(si, back_side);
        }
    ...
    // #Add by Weier
    Float get_eta(const SurfaceInteraction3f &si_,
        Mask active) const override {
        ...
        if (any_or<true>(front_side))
            eta = m_brdf[0]->get_eta(si, front_side);
        ...
        if (any_or<true>(back_side)) {
            ...
            masked(eta, back_side) =
                m_brdf[1]->get_eta(si, back_side);
        }
    ...
    // #Add by Weier
    Float get_inv_eta(const SurfaceInteraction3f &si_,
        Mask active) const override {
        ...
        if (any_or<true>(front_side))
            inv_eta = m_brdf[0]->get_inv_eta(si, front_side);
        ...
        if (any_or<true>(back_side)) {
            ...
            masked(inv_eta, back_side) =
                m_brdf[1]->get_inv_eta(si, back_side);
        }
