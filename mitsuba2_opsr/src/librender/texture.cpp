...
Texture<Float, Spectrum>::pdf_spectrum(const SurfaceInteraction3f &,
                                       Mask) const {
    NotImplementedError("pdf_spectrum");
}

MTS_VARIANT Float Texture<Float, Spectrum>::eval_1(const SurfaceInteraction3f &,
                                                   Mask) const {
...
Texture<Float, Spectrum>::sample_position(const Point2f &sample, Mask) const {
    return { sample, 1.f };
}
...
MTS_VARIANT Volume<Float, Spectrum>::Volume(const Properties &props) {
    m_world_to_local =
        props.transform("to_world", ScalarTransform4f()).inverse();
    update_bbox();
}
...
MTS_VARIANT Float Volume<Float, Spectrum>::eval_1(const Interaction3f &,
                                                  Mask) const {
    NotImplementedError("eval_1");
}
...
Volume<Float, Spectrum>::eval_gradient(const Interaction3f & /*it*/,
                                       Mask /*active*/) const {
    NotImplementedError("eval_gradient");
}

MTS_VARIANT typename Volume<Float, Spectrum>::ScalarFloat
Volume<Float, Spectrum>::max() const {
