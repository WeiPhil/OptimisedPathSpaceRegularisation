 ...
 *  set using the 'roughening_mode' option ('opsr_low','opsr_moderate','opsr_strong','opsr_aggressive')
 * 
...
class PathOPSRLearntIntegrator : public MonteCarloIntegrator<Float, Spectrum> {
public:
    ...
    PathOPSRLearntIntegrator(const Properties &props) : Base(props) {
        m_roughness_res = 4;
        ...
        if (m_roughening_mode == "low") {
            m_att_factors_2d = DynamicBuffer<Float>::copy(
                att_factors_2d_low.data(), att_factors_2d_low.size());
            m_att_factors_3d = DynamicBuffer<Float>::copy(
                att_factors_3d_low.data(), att_factors_3d_low.size());
            m_att_factors_4d = DynamicBuffer<Float>::copy(
                att_factors_4d_low.data(), att_factors_4d_low.size());
            m_att_factors_5d = DynamicBuffer<Float>::copy(
                att_factors_5d_low.data(), att_factors_5d_low.size());
        } else if (m_roughening_mode == "moderate") {
            m_att_factors_2d = DynamicBuffer<Float>::copy(
                att_factors_2d_moderate.data(), att_factors_2d_moderate.size());
            m_att_factors_3d = DynamicBuffer<Float>::copy(
                att_factors_3d_moderate.data(), att_factors_3d_moderate.size());
            m_att_factors_4d = DynamicBuffer<Float>::copy(
                att_factors_4d_moderate.data(), att_factors_4d_moderate.size());
            m_att_factors_5d = DynamicBuffer<Float>::copy(
                att_factors_5d_moderate.data(), att_factors_5d_moderate.size());
        } else if (m_roughening_mode == "strong") {
            m_att_factors_2d = DynamicBuffer<Float>::copy(
                att_factors_2d_strong.data(), att_factors_2d_strong.size());
            m_att_factors_3d = DynamicBuffer<Float>::copy(
                att_factors_3d_strong.data(), att_factors_3d_strong.size());
            m_att_factors_4d = DynamicBuffer<Float>::copy(
                att_factors_4d_strong.data(), att_factors_4d_strong.size());
            m_att_factors_5d = DynamicBuffer<Float>::copy(
                att_factors_5d_strong.data(), att_factors_5d_strong.size());
        } else if (m_roughening_mode == "aggressive") {
            m_att_factors_2d =
                DynamicBuffer<Float>::copy(att_factors_2d_aggressive.data(),
                                           att_factors_2d_aggressive.size());
            m_att_factors_3d =
                DynamicBuffer<Float>::copy(att_factors_3d_aggressive.data(),
                                           att_factors_3d_aggressive.size());
            m_att_factors_4d =
                DynamicBuffer<Float>::copy(att_factors_4d_aggressive.data(),
                                           att_factors_4d_aggressive.size());
            m_att_factors_5d =
                DynamicBuffer<Float>::copy(att_factors_5d_aggressive.data(),
                                           att_factors_5d_aggressive.size());
        } else {
            Log(Error, "One of the roughening mode `low`, `moderate`,`strong` "
                       "or `aggressive` must be chosen");
        }
    ...
    std::pair<Spectrum, Mask> sample(const Scene *scene, Sampler *sampler,
        ...
        for (int depth = 1;; ++depth) {
            ...
            Vector2f acc_roughness = path.get_roughening_opsr(
    ...
    std::string to_string() const override {
        return tfm::format("PathOPSRLearntIntegrator[\n"
                           "  max_depth = %i,\n"
                           "  rr_depth = %i\n"
                           "  roughness_res = %i\n"
                           "  roughening_mode = %s\n"
...
MTS_IMPLEMENT_CLASS_VARIANT(PathOPSRLearntIntegrator, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(PathOPSRLearntIntegrator, "OPSR Path tracer integrator");
