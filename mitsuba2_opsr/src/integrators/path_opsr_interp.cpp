...
/**
 *  IMPORTANT : This integrator is meant to be used to optimise the attenuation factors. 
 *  If you want to use our optimised parameters directly use 'path_opsr_interp_learnt' instead.
 * 
...
class PathOPSRInterpIntegrator : public MonteCarloIntegrator<Float, Spectrum> {
public:
    ...
    PathOPSRInterpIntegrator(const Properties &props) : Base(props) {
        m_roughness_res = 4;
 
        m_regularise_all = props.bool_("regularise_all", false); 

        m_att_factors_2d = zero<DynamicBuffer<Float>>();
        m_att_factors_3d = zero<DynamicBuffer<Float>>();
        m_att_factors_4d = zero<DynamicBuffer<Float>>();
        m_att_factors_5d = zero<DynamicBuffer<Float>>();
    ...
    void traverse(TraversalCallback *callback) override { 
        callback->put_parameter("att_factors_2d", m_att_factors_2d);
        callback->put_parameter("att_factors_3d", m_att_factors_3d);
        callback->put_parameter("att_factors_4d", m_att_factors_4d);
        callback->put_parameter("att_factors_5d", m_att_factors_5d); 
    }

    std::string to_string() const override {
        return tfm::format("PathOPSRInterpIntegrator[\n"
                           "  max_depth = %i,\n"
                           "  rr_depth = %i\n"
                           "  m_att_factors_2d = %s"
                           "  m_att_factors_3d = %s"
                           ...
                           "  regularise_all = %i\n"
                           "]",
                           m_max_depth, m_rr_depth, m_att_factors_2d,
                           m_att_factors_3d, m_att_factors_4d, m_att_factors_5d,
                           m_roughness_res, m_regularise_all);
    }
...
MTS_IMPLEMENT_CLASS_VARIANT(PathOPSRInterpIntegrator, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(PathOPSRInterpIntegrator, "OPSR Interp Path integrator");
