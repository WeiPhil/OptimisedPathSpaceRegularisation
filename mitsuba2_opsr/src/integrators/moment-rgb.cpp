...
RGB Moment integrator (:monosp:`moment`)
-----------------------------------------------------
 ...
 * - (Nested plugin)
   ...
   - Sub-integrators (can have more than one) which will be sampled along the
AOV integrator. Their respective XYZ output will be put into distinct images.


This integrator returns one AOVs recording the second moment of the samples of
the nested integrator.

 */

template <typename Float, typename Spectrum>
class MomentRGBIntegrator final : public SamplingIntegrator<Float, Spectrum> {
public:
    MTS_IMPORT_BASE(SamplingIntegrator)
    MTS_IMPORT_TYPES(Scene, Sampler, Medium)

    MomentRGBIntegrator(const Properties &props) : Base(props) {
    ...
    std::pair<Spectrum, Mask> sample(const Scene *scene, Sampler *sampler,
        ...
        for (size_t i = 0; i < m_integrators.size(); i++) {
            std::pair<Spectrum, Mask> result_sub =
                m_integrators[i].first->sample(scene, sampler, ray, nullptr,
                                               aovs, active);
            ...
            if constexpr (is_monochromatic_v<Spectrum>) {
                rgb = luminance(spec_u);
            } else if constexpr (is_rgb_v<Spectrum>) {
                rgb = spec_u;
            } else {
                static_assert(is_spectral_v<Spectrum>);
                /// Note: this assumes that sensor used sample_rgb_spectrum() to
                /// generate 'ray.wavelengths'
                auto pdf = pdf_rgb_spectrum(ray.wavelengths);
                spec_u *= select(neq(pdf, 0.f), rcp(pdf), 0.f);
                rgb = xyz_to_srgb(
                    spectrum_to_xyz(spec_u, ray.wavelengths, active));
            }
    ...
    std::string to_string() const override {
        ...
        for (size_t i = 0; i < m_integrators.size(); ++i) {
            oss << "    "
                << string::indent(m_integrators[i].first->to_string(), 4);
...
MTS_IMPLEMENT_CLASS_VARIANT(MomentRGBIntegrator, SamplingIntegrator)
MTS_EXPORT_PLUGIN(MomentRGBIntegrator, "Moment RGB integrator");
